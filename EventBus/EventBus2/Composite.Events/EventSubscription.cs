    ...
    public class EventSubscription<TPayload> : IEventSubscription
    {
        ...
        public EventSubscription(IDelegateReference actionReference, IDelegateReference filterReference)
        {
            ...
            if (!(actionReference.Target is Action<TPayload>))
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, "Invalid delegate rerefence type {0} {1}", typeof(Action<TPayload>).FullName), "actionReference");

            if (filterReference == null)
                throw new ArgumentNullException("filterReference");
            if (!(filterReference.Target is Predicate<TPayload>))
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, "Invalid delegate rerefence type {0} {1}", typeof(Predicate<TPayload>).FullName), "filterReference");
        ...
        /// <summary>
        /// Gets or sets a <see cref="global::EventBus.Composite.Events.SubscriptionToken"/> that identifies this <see cref="IEventSubscription"/>.
        /// </summary>
